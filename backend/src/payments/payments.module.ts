...
export class PaymentsService {
  ...
  async getMoraInfo(loanId: string) {
    ...
    for (const s of schedules) {
      ...
      if (gen > 0) {
        ...
        const dias = this.moraService.businessDaysOverdue(
          new Date(s.dueDate),
          today,
        );
        detalle.push({
  ...
  async register(
    ...
    if (!loan) throw new NotFoundException("Préstamo no encontrado");
    if (
      ![LoanStatus.ACTIVO, LoanStatus.VENCIDO, LoanStatus.ATRASADO].includes(
    ...
    if (paymentType === "MORATORIO") {
      ...
      for (const schedule of targetSchedules) {
        ...
        schedule.balanceDue = this.calculator.round(
          Math.max(0, Number(schedule.balanceDue) - applyAmount),
        );
        schedule.status =
          schedule.balanceDue <= 0
            ? ScheduleStatus.PAGADO
            : ScheduleStatus.PARCIAL;
    ...
    const countResult = await this.dataSource.query(
      "SELECT COUNT(*) as cnt FROM pagos WHERE fecha_pago >= ?",
      [todayStart],
    );
...
const liquidado = nuevoEstado === LoanStatus.LIQUIDADO;
  ...
  async getGeoPayments(date?: string) {
    ...
    const collectorIds = [
      ...new Set(rows.map((p: any) => p.collectorId).filter(Boolean)),
    ];
  ...
  async generateThermalTicket(paymentId: string, res: Response): Promise<void> {
    ...
    return this.pdfService.generateThermalReceipt(
      { payment, loan, company, stats },
      res,
    );
  }
...
export class PaymentsController {
  ...
  register(@Body() dto: any, @CurrentUser("id") userId: string) {
    const source =
      dto.source === "COBRADOR" ? PaymentSource.COBRADOR : PaymentSource.CAJA;
  ...
  @Get("cuotas-con-mora/:loanId")
  @AuthPermission("moratorios.eliminar")
