      ...
      @if (pagos().length > 0) {
        <div class="totales">
          ...
          <div class="tot-card">
            ...
            <span class="tot-val">{{
              totalCobrado() | currency: "MXN" : "symbol" : "1.2-2"
            }}</span>
          </div>
        </div>
      }
        ...
        @if (loading()) {
          ...
          <table mat-table [dataSource]="pagosFiltrados()" class="tabla">
            ...
            <ng-container matColumnDef="monto">
              ...
              <td mat-cell *matCellDef="let p" class="r mono">
                {{ p.amountPaid | currency: "MXN" : "symbol" : "1.2-2" }}
              </td>
            </ng-container>
            <ng-container matColumnDef="acciones">
              ...
              <td mat-cell *matCellDef="let p" class="r">
                ...
                <button
                  mat-icon-button
                  color="primary"
                  (click)="printTicket(p.id)"
                  matTooltip="Reimprimir ticket (80mm)"
                >
                  <mat-icon>receipt_long</mat-icon>
                </button>
                ...
                <button
                  ...
                  [matTooltip]="
                    telefonoDe(p) ? 'Enviar por WhatsApp' : 'Sin teléfono'
                  "
                >
...
export class PaymentsQueryComponent implements OnInit {
  ...
  async consultar() {
    ...
    if (this.modo === "dia") {
      if (!this.fechaDia) {
        this.snackbar.open("Elige una fecha", "Cerrar", { duration: 3000 });
        return;
      }
      from = this.toISO(this.fechaDia);
    } else {
      if (!this.fechaDesde || !this.fechaHasta) {
        this.snackbar.open("Elige ambas fechas", "Cerrar", { duration: 3000 });
        return;
      }
      from = this.toISO(this.fechaDesde);
      to = this.toISO(this.fechaHasta);
      if (to < from) {
        this.snackbar.open("La fecha final no puede ser anterior", "Cerrar", {
          duration: 3500,
        });
        return;
      }
    ...
    try {
      ...
      const res = await firstValueFrom(
        this.http.get<any>(`${this.base}/payments/by-range?${qs}`),
      );
  ...
  async downloadReceipt(id: string) {
    try {
      const blob = await firstValueFrom(
        this.http.get(`${this.base}/payments/${id}/receipt`, {
          responseType: "blob",
        }),
      );
      ...
      this.snackbar.open("No se pudo descargar el comprobante", "Cerrar", {
        duration: 4000,
      });
    }
  }
...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      <div class="row">
        <span>Folio</span><strong>{{ p.receiptNumber || "—" }}</strong>
      </div>
      <div class="row">
        <span>Cliente</span
        ><strong>{{ p.loan?.customer?.fullName || "—" }}</strong>
      </div>
      <div class="row">
        <span>Fecha</span
      ...
      @if (num(p.lateInterestApplied) > 0) {
        <div class="row">
          <span>Moratorio</span
          ><strong>{{ p.lateInterestApplied | currency: "MXN" }}</strong>
        </div>
      }
      <div class="row total">
        <span>Total recibido</span
        ><strong>{{ p.amountPaid | currency: "MXN" }}</strong>
      </div>
      @if (cuotas().length > 0) {
        <hr />
  ...
  cuotas(): number[] {
    ...
    try {
      ...
      return arr.map((x: any) =>
        typeof x === "object" ? (x.periodo ?? x.period ?? x) : x,
      );
    } catch {
      return [];
    }
