...
@Component({
  ...
  template: `
    <div class="page-header">
      ...
      @if (auth.can("cobranza.asignar")) {
        <a
          mat-raised-button
          color="primary"
          routerLink="/collection/assignments"
        >
    ...
    <div class="kpi-row-dash">
      <div class="kpi-dash kpi-blue">
        <div class="kpi-dash-left">
          ...
          <div class="kpi-dash-value">
            {{ totalActivos() + totalVencidos() + totalAtrasados() }}
          </div>
          <div class="kpi-dash-sub">
            <span class="kpi-arrow up">↑</span>
            &nbsp;Activos + Atrasados + Vencidos
          </div>
        </div>
      ...
      <div class="kpi-dash kpi-red">
        <div class="kpi-dash-left">
          ...
          <div class="kpi-dash-sub">
            <span class="kpi-arrow down">↑</span>&nbsp;Requieren atención
          </div>
        </div>
        <div class="kpi-dash-icon"><mat-icon>warning</mat-icon></div>
      </div>

      <div class="kpi-dash kpi-orange">
        <div class="kpi-dash-left">
          <div class="kpi-dash-label">Créditos atrasados</div>
      ...
      <div class="kpi-dash kpi-green">
        <div class="kpi-dash-left">
          ...
          <div class="kpi-dash-sub">
            <span class="kpi-arrow up">↑</span>&nbsp;Asignados
          </div>
        </div>
      ...
      <div class="kpi-dash kpi-amber">
        <div class="kpi-dash-left">
          ...
          <div class="kpi-dash-sub">
            <span class="kpi-arrow down">↑</span>&nbsp;Sin asignar
          </div>
        </div>
    ...
    <mat-tab-group>
            ...
            @if (loading()) {
              ...
              <table mat-table [dataSource]="vencidos()">
                ...
                <ng-container matColumnDef="amount">
                  ...
                  <td mat-cell *matCellDef="let r">
                    {{ r.principalAmount | currency: "MXN" }}
                  </td>
                </ng-container>
                <ng-container matColumnDef="payment">
                  <th mat-header-cell *matHeaderCellDef>Cuota</th>
                  <td mat-cell *matCellDef="let r">
                    {{ r.periodicPayment | currency: "MXN" }}
                  </td>
                </ng-container>
                <ng-container matColumnDef="cobrador">
                  ...
                  <td mat-cell *matCellDef="let r">
                    <span
                      [style.color]="r.collectorId ? '#16A34A' : '#DC2626'"
                ...
                <ng-container matColumnDef="actions">
                  ...
                  <td mat-cell *matCellDef="let r">
                    <a
                      mat-icon-button
                      [routerLink]="['/loans', r.id]"
                      matTooltip="Ver crédito"
                    >
              ...
              <mat-paginator
                [length]="totalVencidos()"
                [pageSize]="pageSize"
                [pageSizeOptions]="[10, 20, 50]"
                (page)="onPageVencidos($event)"
              >
              </mat-paginator>
            }
          </mat-card-content>
      ...
      <mat-tab label="Cartera atrasada ({{ totalAtrasados() }})">
        <mat-card style="margin-top:16px">
          <mat-card-content>
            @if (loadingAtrasados()) {
              <div class="loading-overlay">
              ...
              <div class="empty-state">
                <mat-icon>schedule</mat-icon>
                <p>Sin créditos atrasados</p>
              </div>
            } @else {
              <table mat-table [dataSource]="atrasados()">
                <ng-container matColumnDef="customer">
                  <th mat-header-cell *matHeaderCellDef>Cliente</th>
                  <td mat-cell *matCellDef="let r">
              ...
              <mat-paginator
                [length]="totalAtrasados()"
                [pageSize]="pageSizeAtrasados"
                [pageSizeOptions]="[10, 20, 50]"
                (page)="onPageAtrasados($event)"
              >
              </mat-paginator>
            }
          </mat-card-content>
        ...
        <mat-card style="margin-top:16px">
          <mat-card-content>
            @if (loadingActivos()) {
              ...
              <mat-paginator
                [length]="totalActivos()"
                [pageSize]="pageSizeActivos"
                [pageSizeOptions]="[10, 20, 50]"
                (page)="onPageActivos($event)"
              >
              </mat-paginator>
            }
          </mat-card-content>
  ...
  styles: [
    `
      ...
      .kpi-blue {
        background: linear-gradient(135deg, #4f7af8 0%, #6b5ce7 100%);
      }
      .kpi-red {
        background: linear-gradient(135deg, #ef4444 0%, #b91c1c 100%);
      }
      .kpi-green {
        background: linear-gradient(135deg, #1c4532 0%, #245c3e 100%);
      }
      .kpi-amber {
...
export class CollectionDashboardComponent implements OnInit {
  ...
  loadingAtrasados = signal(false);
  atrasados = signal<any[]>([]);
  totalAtrasados = signal(0);

  pageAtrasados = 0;
  pageSizeAtrasados = 20; // o el tamaño que utilices
  ...
  this.loadingAtrasados.set(true);
  ...
  loadActivos() {
    ...
    this.api
      .get<any>("/loans", {
        status: "ACTIVO",
        page: this.pageActivos + 1,
        limit: this.pageSizeActivos,
      })
      .subscribe({
        next: (r) => {
          const data = Array.isArray(r) ? r : (r?.data ?? []);
  ...
  loadAtrasados() {
    ...
    this.api
      .get<any>("/loans", {
        status: "ATRASADO",
        page: this.pageAtrasados + 1,
        limit: this.pageSizeAtrasados,
      })
      .subscribe({
        next: (r) => {
          const data = Array.isArray(r) ? r : (r?.data ?? []);
          ...
          this.atrasados.set(data);
          this.totalAtrasados.set(total);

          this.loadingAtrasados.set(false);
          this.updateKpis();
        },
        error: () => this.loadingAtrasados.set(false),
      });
  }
 ...
 updateKpis() {
  ...
  this.withCollector.set(
    todos.filter((l: any) => l.collectorId).length
  );

  this.sinCobrador.set(
    todos.filter((l: any) => !l.collectorId).length
  );
}

  onPageVencidos(e: PageEvent) {
    this.page = e.pageIndex;
    this.pageSize = e.pageSize;
    this.loadVencidos();
  }
  onPageActivos(e: PageEvent) {
    this.pageActivos = e.pageIndex;
    this.pageSizeActivos = e.pageSize;
    this.loadActivos();
  }

  onPageAtrasados(e: PageEvent) {
    this.pageAtrasados = e.pageIndex;
    this.pageSizeAtrasados = e.pageSize;
    this.loadAtrasados();
  }
}
